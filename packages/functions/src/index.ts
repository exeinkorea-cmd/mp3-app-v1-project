...
// Firebase Admin 초기화 (lazy initialization으로 변경)
...
async function performDailyReset(): Promise<void> {
  ...
  try {
    ...
    try {
      // authCheckIns 컬렉션에서 활성 사용자 목록 가져오기
      const checkInsSnapshot = await getDb().collection("authCheckIns").get();
      ...
      for (const userId of userIds) {
        try {
          ...
          logger.info(`사용자 ${userId} 세션 무효화 완료`);
        } catch (error) {
          logger.error(`사용자 ${userId} 세션 무효화 실패:`, error);
        }
      }
      logger.info(`총 ${userIds.size}명의 사용자 세션 무효화 완료`);
    } catch (error) {
    ...
    try {
      const bulletinsSnapshot = await getDb().collection("bulletins").get();
      const now = new Date();
    ...
    try {
      const alertsSnapshot = await getDb().collection("emergencyAlerts").get();
      const deleteBatches: WriteBatch[] = [];
      let currentBatch = getDb().batch();
      let batchCount = 0;
    ...
    try {
      ...
      do {
        ...
        const revokePromises = listUsersResult.users.map(async (userRecord) => {
          try {
            await getAuthInstance().revokeRefreshTokens(userRecord.uid);
            return 1;
          } catch (error) {
...
async function checkAttendanceStatus(checkTime: string) {
  ...
  try {
      ...
      if (distance <= SITE_CENTER.radiusMeters) {
        ...
        if (lastPrompt && lastPrompt.timestamp) {
          ...
          if (minutesDiff >= 30) {
            ...
            // checkOutTime 업데이트
            await getDb().collection("authCheckIns").doc(user.docId).update({
    ...
    for (const user of siteOutsideUsers) {
      ...
      await getDb().collection("checkoutPrompts").add({
        userId: user.userId,
        userName: user.userName,
        timestamp: FieldValue.serverTimestamp(),
        message: "퇴근 하시겠습니까?",
