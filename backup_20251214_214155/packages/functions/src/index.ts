...
// ============================================================================
// [Critical Fix] Firebase Admin 전역 초기화
// 이 코드는 파일이 로드될 때 무조건 한 번 실행됩니다.
// ============================================================================
try {
  if (admin.apps.length === 0) {
    admin.initializeApp();
    logger.info("🔥 Firebase Admin Globally Initialized");
  } else {
    logger.info("✅ Firebase Admin already initialized");
  }
} catch (initError) {
  logger.error("❌ Global Firebase Admin initialization failed:", initError);
  // 전역 초기화 실패해도 계속 진행 (함수 내부에서 재시도)
}
...
function getAppInstance(): App {
  if (!appInstance) {
    // admin.apps를 직접 확인하고, 없으면 초기화
    if (admin.apps.length === 0) {
      appInstance = admin.initializeApp();
    } else {
      // getApp() 대신 admin.app() 사용 (더 안전 - 기본 앱이 없어도 에러 안 남)
      try {
        appInstance = admin.app();
      } catch (err) {
        // admin.app()이 실패하면 초기화 후 다시 시도
        logger.warn("admin.app() failed, initializing new app:", err);
        appInstance = admin.initializeApp();
      }
    }
  }
      ...
      try {
        ...
        if (!bulletinsSnapshot.empty) {
          ...
          bulletinsSnapshot.docs.forEach((doc, index) => {
            ...
            if (count >= 400 || index === bulletinsSnapshot.docs.length - 1) {
              batches.push(currentBatch);
              if (index < bulletinsSnapshot.docs.length - 1 && count >= 400) {
                currentBatch = db.batch();
                count = 0;
              }
            }
        ...
        try {
          ...
          snapshot.docs.forEach((doc, index) => {
            ...
            if (count >= 400 || index === snapshot.docs.length - 1) {
              batches.push(currentBatch);
              if (index < snapshot.docs.length - 1 && count >= 400) {
                currentBatch = db.batch();
                count = 0;
              }
            }
 ...
 * - 격리: 외부 헬퍼 함수에 의존하지 않는 완전 독립 함수
 */
export const manualRevokeSessions = onCall(
  { region: "us-central1" },
  async (request) => {
    logger.info("🔥 [System] 로직 시작: 앱 초기화 상태 확인");

    try {
      // 1. [격리된 초기화] 외부 헬퍼 믿지 않음, 직접 확인
      if (admin.apps.length === 0) {
        logger.info("⚠️ 앱이 초기화되지 않음 -> 지금 초기화 수행");
        admin.initializeApp();
        logger.info("✅ Firebase Admin 초기화 완료");
      } else {
        logger.info("✅ 앱이 이미 초기화되어 있음");
      }

      // 2. [인스턴스 직접 생성] 전역 변수 사용 안 함
      ...
      logger.info(`🔐 ${userIds.size}명의 사용자 토큰을 만료합니다.`);

      // 6. 안전한 토큰 만료 처리 (실패해도 계속 진행)
      let tokenRevokeSuccess = 0;
      let tokenRevokeError = 0;

      const tokenRevokePromises = Array.from(userIds).map(async (uid) => {
        try {
          ...
          tokenRevokeError++;
          logger.error(`토큰 만료 실패 (UID: ${uid}):`, err);
          return { success: false, uid, error: err };
        }
      });

      await Promise.all(tokenRevokePromises);

      logger.info(
        `🔐 토큰 만료 완료: 성공 ${tokenRevokeSuccess}명, 실패 ${tokenRevokeError}명`
      );

      // 7. [배치 삭제] 400개 단위 Chunking
      ...
      logger.info(`✅ 데이터 삭제 완료: 총 ${snapshot.size}건`);

      return {
        success: true,
        message: `총 ${snapshot.size}건의 출석 데이터가 삭제되었습니다.`,
      ...
      // 에러 객체를 문자열로 변환하여 상세히 반환
      throw new Error(
        `로그아웃 처리 중 서버 오류가 발생했습니다: ${errorMessage}`
      );
    }
  }
);
  ...
  try {
    ...
    try {
      ...
      if (configDoc.exists) {
        ...
        siteConfig = {
          center: {
            latitude: configData?.center?.latitude || DEFAULT_SITE_CONFIG.center.latitude,
            longitude: configData?.center?.longitude || DEFAULT_SITE_CONFIG.center.longitude,
          },
          allowedRadiusMeters: configData?.allowedRadiusMeters || DEFAULT_SITE_CONFIG.allowedRadiusMeters,
        };
      } else {
